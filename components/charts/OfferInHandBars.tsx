...
export function OfferInHandBars({ rows }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-border bg-surface p-4">
      <p className="text-sm font-medium text-foreground">Monthly in-hand (relative)</p>
      <ul className="space-y-2.5">
        {rows.map((r, i) => {
          ...
          return (
            <li key={`${r.label}-${i}`}>
              <div className="mb-1 flex justify-between gap-2 text-xs text-foreground-secondary">
