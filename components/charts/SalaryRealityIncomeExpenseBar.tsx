...
export function SalaryRealityIncomeExpenseBar({ inHandMonthly, lines, monthlySavings }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-border bg-surface p-4">
      <p className="text-sm font-medium text-foreground">In-hand vs modeled spend</p>
      <p className="text-xs text-foreground-muted">
        Each segment is share of <strong>estimated monthly in-hand</strong> — a planning view, not accounting.
      </p>
      <div className="flex h-10 w-full overflow-hidden rounded-xl bg-border/30">
        {lines.map((line, i) => {
      ...
      <ul className="grid gap-1 text-xs text-foreground-secondary sm:grid-cols-2">
        ...
        <li>
          <span className="font-medium text-foreground">Modeled spend:</span>{" "}
